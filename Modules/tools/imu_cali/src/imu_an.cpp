...
imu_callback( const sensor_msgs::ImuConstPtr& imu_msg )
{
    ...
    {
        double time_min = ( time - start_t ) / 1.0;
        if ( time_min > max_time_s ){
            end = true;
            std::cout << "recieve end" << std::endl;
        }
    }
}
...
main( int argc, char** argv )
{
    ...
    max_time_s   = ros_utils::readParam< double >( n, "max_time_s" );
