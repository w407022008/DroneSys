#include "inner_loop_controller/inner_loop_controller.h"
...
namespace inner_loop_controller {
...
RPGRotorsInterface::RPGRotorsInterface(const ros::NodeHandle& nh,
  ...
  // from agent (e.g gazebo)
  odometry_sub_ = nh_.subscribe(
      "odometry", 1, &RPGRotorsInterface::odometryCallback, this);
  // from agent (e.g gazebo)
  motor_speed_sub_ = nh_.subscribe(
      "motor_speed", 1, &RPGRotorsInterface::motorSpeedCallback, this);
  
  // from station (e.g rqt_gui)
  arm_interface_sub_ =
      nh_.subscribe("inner_loop_controller/arm", 1,
...
void RPGRotorsInterface::odometryCallback(
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "inner_loop_controller");
  ...
  // Run the interface
  inner_loop_controller::RPGRotorsInterface inner_loop_controller;
  ros::spin();
