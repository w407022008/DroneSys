...
void processStereo(const sensor_msgs::ImageConstPtr& msgLeft, const sensor_msgs::ImageConstPtr& msgRight)//, const geometry_msgs::PoseStampedConstPtr& msgPose)
{
    ...
    //StereoPointCloud stereoPointCloud;
    // 将计算出的深度图转为ROS的图像消息
    ROS_INFO("run stereo match");
    ros::Time time = ros::Time::now();
    cv::Mat depthImage = stereoMatch.run(leftImage, rightImage, baseline, fx);
    ROS_INFO("time consumming: %f",(ros::Time::now()-time).toSec());
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    // 参数读取
    nh.param<float>("elas_stereo_node/focal_length", fx, 711.9);
    nh.param<float>("elas_stereo_node/base_line", baseline, 0.12);
    std::string focal_length, leftTopic, rightTopic, poseTopic;
    ...
    // // 话题同步，进行回调
    // typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::Image, geometry_msgs::PoseStamped> SyncPol;
    // message_filters::Synchronizer<SyncPol> sync(SyncPol(10), left_sub, right_sub, pose_sub);
    // sync.registerCallback(boost::bind(&processStereo, _1, _2, _3));
