...
void publish_odometry(const ros::Publisher & pubOdomAftMapped)
{
    ...
    br.sendTransform( tf::StampedTransform( transform, odomAftMapped.header.stamp, "world", "lio_imu") );
}
...
int main(int argc, char** argv)
{
    ...
    // ros::Publisher pubLaserCloudEffect  = nh.advertise<sensor_msgs::PointCloud2>
            // ("/cloud_effected", 1000);
    ros::Publisher pubInitMap = nh.advertise<sensor_msgs::PointCloud2>
            ("/cloud_uninitialized", 1000);
...
//------------------------------------------------------------------------------------------------------
    ...
    while (status)
    {
        ...
        if(sync_packages(Measures)) 
        {
            ...
            double t0,t1,t2,t3,t4,match_start, solve_start;
            match_time = 0;
            solve_time = 0;
            propag_time = 0;
            update_time = 0;
            ...
            t1 = omp_get_wtime();
            p_imu->Process(Measures, feats_undistort); // rot_init by imu sampling
            ...
            {
                time_seq = time_compressing<int>(feats_down_body); // skip time_seq incorrect one even if sort
            ...
            if(!init_map)
            {
                ...
                {   
                    ...
                    publish_init_map(pubInitMap); //(pubLaserCloudFullRes);
            ...
            else
            {
                ...
                effct_feat_num = 0;
                if (time_seq.size() > 0) // must be
                {
                    double pcl_beg_time = Measures.lidar_beg_time;
                    idx = -1;
                    for (k = 0; k < time_seq.size(); k++) // as points_size
                    {
                        PointType &point_body  = feats_down_body->points[idx+time_seq[k]];
                        time_current = point_body.curvature / 1000.0 + pcl_beg_time;
                        if (is_first_frame)
            ...
            if (runtime_pos_log)
            {
                ...
                s_plot[time_log_counter] = t4 - t0;
                s_plot2[time_log_counter] = feats_undistort->points.size();
                s_plot3[time_log_counter] = aver_time_consu;
                time_log_counter ++;
                std::cout<<fixed<<setprecision(6)<<
                "[ mapping ]: time (s): IMU + Map + Input Downsample: "<<t1-t0<<
                " icp: "<<t3-t1<<
                " map incre: "<<t4-t3<<
                " ave total: "<<aver_time_consu<<
                " ave match: "<<aver_time_match<<
                " ave solve: "<<aver_time_solve<<
                " ave ICP: "<<aver_time_icp<<
                " ave propag: "<<aver_time_propag<<std::endl;
