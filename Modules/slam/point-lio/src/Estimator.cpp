...
esekfom::esekf<state_output, 30, input_ikfom> kf_output; // input without imu, so omg & acc should be est
...
// state_input s: pos(0-2) rot(3-5) offset_R(6-8) offset_T(9-11) vel(12-14) bg(15-17) ba(18-20) g(21-23)
// input_ikfom u: acc(0-2) gyro(3-5)
// output s.dot: vel(0-2) omega(3-5) keep keep acc(12-14) keep keep keep
...
// output: ekfom_data.hx ekfom_data.z
void h_model_input(state_input &s, Eigen::Matrix3d cov_p, Eigen::Matrix3d cov_R, esekfom::dyn_share_modified<double> &ekfom_data)
{
	...
	VF(4) pabcd; // vec_normalized norm of original vec_s
	pabcd.setZero();
	...
	for (int j = 0; j < time_seq[k]; j++)
	{
		...
		V3D p_body = pbody_list[idx+j+1];//feats_down_body actually
		...
		p_world << point_world_j.x, point_world_j.y, point_world_j.z;
		{
			...
			{
				point_selected_surf[idx+j+1] = false;
				if (esti_plane(pabcd, points_near, plane_thr)) //(planeValid) // get norm vector of all near points vector by pseudo-inverse 
				{
					float pd2 = fabs(pabcd(0) * point_world_j.x + pabcd(1) * point_world_j.y + pabcd(2) * point_world_j.z + pabcd(3)); // it<plane_thr means flatten enough
					// V3D norm_vec;
					// M3D Rpf, pf;
					...
					// }
					// std::cout<<p_norm<<", "<<match_s * pd2 * pd2<<(p_norm > match_s * pd2 * pd2?", ":", False")<<std::endl;
					if (p_norm > match_s * pd2 * pd2)
					{
	...
	ekfom_data.h_x.resize(effect_num_k, 12); // state.top12: pos, rot, offset_R, offset_T
	ekfom_data.h_x = Eigen::MatrixXd::Zero(effect_num_k, 12);
	ekfom_data.z.resize(effect_num_k);
	int m = 0;
	std::cout<<effect_num_k<<std::endl;
	for (int j = 0; j < time_seq[k]; j++)
	{
