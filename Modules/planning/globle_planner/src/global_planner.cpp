...
void Global_Planner::track_path_cb(const ros::TimerEvent& e)
{
    ...
    //     // 但如何脱离该点呢？
    //     pub_message(message_pub, drone_msgs::Message::WARN, NODE_NAME, "Drone Position Dangerous! STOP HERE and wait for new goal.");
    ...
    switch (exec_state)
    {
        ...
        case PLANNING:
        {
            ...
            if(astar_state==Astar::NO_PATH)
            {
                path_ok = false;
                exec_state = EXEC_STATE::WAIT_GOAL;
                pub_message(message_pub, drone_msgs::Message::WARN, NODE_NAME, "Planner can't find path, please reset the goal!");
            }
            else
            {
                path_ok = true;
                ...
                pub_message(message_pub, drone_msgs::Message::NORMAL, NODE_NAME, "Get a new path!");  
            }
