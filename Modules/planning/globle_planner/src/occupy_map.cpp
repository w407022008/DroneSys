...
void Occupy_map::inflate_point_cloud(void)
{
    if(!has_global_point)
    { 
        pub_message(message_pub, drone_msgs::Message::WARN, NODE_NAME, "Occupy_map [inflate point cloud]: don't have global point, can't inflate!\n");
        return;
    }
...
void Occupy_map::setOccupancy(Eigen::Vector3d pos, int occ) 
{
    if (occ != 1 && occ != 0) 
    {
        pub_message(message_pub, drone_msgs::Message::WARN, NODE_NAME, "occ value error!\n");
        return;
    }
...
bool Occupy_map::check_safety(Eigen::Vector3d& pos, double check_distance)
{
    ...
    {
        // 当前位置点不在地图内
        pub_message(message_pub, drone_msgs::Message::WARN, NODE_NAME, "[check_safety]: the odom point is not in map\n");
        return 0;
    }
    Eigen::Vector3i id;
    posToIndex(pos, id);
        ...
        for(int iy=-check_dist_xy; iy<=check_dist_xy; iy++){
            for(int iz=-check_dist_z; iz<=check_dist_z; iz++){
                ...
                if(!isInMap(pos_occ)){
                    ...
                    // pub_message(message_pub, drone_msgs::Message::WARN, NODE_NAME, "[check_safety]: current odom is near the boundary of the map\n");
                    return 0;
                }
