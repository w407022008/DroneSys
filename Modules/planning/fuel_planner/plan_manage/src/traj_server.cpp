...
ros::Publisher cmd_vis_pub, pos_cmd_pub, command_pub, traj_pub;
...
// output experiment's result
const char*  home_dir = std::getenv("HOME");
string file_path = std::string(home_dir) + "/sim_ws/wuhan/fuel_ref/src/experiments/result.txt";
bool finish = false;

// control MAV initial pose by keyboard
...
void publish(
  ...
  double yaw = 0.0, double yaw_dot = 0.0, ros::Time time=ros::Time::now()){

  if (pos_cmd_pub.getNumSubscribers() > 0){
  ...
  if (command_pub.getNumSubscribers() > 0){
    ...
    Command_Now.Reference_State.position_ref[0]     = p(0);
    Command_Now.Reference_State.position_ref[1]     = p(1);
    Command_Now.Reference_State.position_ref[2]     = p(2);
    Command_Now.Reference_State.velocity_ref[0]     = v(0);
    Command_Now.Reference_State.velocity_ref[1]     = v(1);
    Command_Now.Reference_State.velocity_ref[2]     = v(2);
    Command_Now.Reference_State.yaw_ref             = yaw;
...
void keyboardControlCallback(std_msgs::String msg){
  ...
  if (msg.data == "i") {
    if (keyboard_control) {
      ...
      for (int i = 0; i < 100; ++i) {
        pos(2) += 0.01;
        yaw += M_PI/100;
        publish(pos, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(),yaw);
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  ros::Time time_now = ros::Time::now();
  // double init_yaw = wrapYaw(traj_[3].evaluateDeBoorT(0)[0]);
  // double delt_yaw = tf2::getYaw(odom.pose.pose.orientation) - init_yaw;
  // delt_yaw = delt_yaw > M_PI ? 2*M_PI-delt_yaw : (delt_yaw<-M_PI ? delt_yaw+2*M_PI : fabs(delt_yaw));
  // if (new_traj){
  //   if (delt_yaw > M_PI/2){
  //     Eigen::Vector3d pos(odom.pose.pose.position.x,
  //                         odom.pose.pose.position.y,
  ...
  //     publish(pos, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(), init_yaw, 0, time_now);
  //     ROS_INFO("[cmdCallback]: Hovering and tracking yaw_ref.=========================================");
  //     return;
  //   }
...
static Eigen::Vector3d last_pos = Eigen::Vector3d::Zero();
  if (t_cur < traj_duration_ && t_cur >= 0.0) {
    ...
    yaw = wrapYaw(traj_[3].evaluateDeBoorT(t_cur)[0]);
    ...
    vel.setZero();
    acc.setZero();
    yaw = wrapYaw(traj_[3].evaluateDeBoorT(traj_duration_)[0]);
    yawdot = 0.0;
    ...
    if(!finish){
      ...
      file << "flight time: " << flight_t << "\n"
           << "path length: " << len << "\n" 
           << "mean vel: " << len /flight_t << "\n"
           << "energy is: " << energy << std::endl;
      finish = true;     
    }                   
  } else {
    cout << "[Traj server]: invalid time."<<t_cur << endl;
  }
...
last_pos=pos;
  publish(pos, vel, acc, yaw, yawdot, time_now);
// std::cout<<"[traj_server]yaw: "<<yaw<<std::endl;
  // Draw
  Eigen::Vector3d dir(cos(yaw), sin(yaw), 0.0);
  // drawCmd(pos, vel, 0, Eigen::Vector4d(0, 1, 0, 1));
  drawCmd(pos, 2 * dir, 1, Eigen::Vector4d(1, 1, 0, 0.7));
  // drawCmd(pos, acc, 2, Eigen::Vector4d(0, 0, 1, 1));
  // drawCmd(pos, pos_err, 3, Eigen::Vector4d(1, 1, 0, 0.7));
  percep_utils_->setPose(pos, wrapYaw(yaw-M_PI));
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber keyborad_sub = node.subscribe("/keyboard/control", 1, keyboardControlCallback);

  cmd_vis_pub = node.advertise<visualization_msgs::Marker>("planning/position_cmd_vis", 10);
  pos_cmd_pub = node.advertise<quadrotor_msgs::PositionCommand>("/position_cmd", 50);
  command_pub = node.advertise<drone_msgs::ControlCommand>("/control_command", 50);
