...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  ...
  nh.param("manager/use_geometric_path", use_geometric_path, false);// all are true
  ...
  if (use_optimization) {
    int optimizer = 2;
    bspline_optimizers_.resize(optimizer);
    for (int i = 0; i < optimizer; ++i) {
  ...
  if (use_active_perception) {
    ...
    // heading_planner_.reset(new HeadingPlanner(nh)); // nothing used here
    // heading_planner_->setMap(sdf_map_);
    // visib_util_.reset(new VisibilityUtil(nh)); // used in topo_fsm for visibility cons in opt
...
// reparam bspline to bspline according to dt & lengthen_ratio
...
bool FastPlannerManager::kinodynamicReplanFromTo(const Eigen::Vector3d& start_pt,
  ...
  std::cout << "[kinoPlan]: start: " << start_pt.transpose() << ", " << start_vel.transpose()
  ...
  std::cout << "[kinoPlan]: State error: (" << (start2[0] - start[0]).norm() << ", "
            << (start2[1] - start[1]).norm() << ", " << (start2[2] - start[2]).norm() << ")"
            << std::endl;

  double t_opt = (ros::Time::now() - t1).toSec();
  ROS_WARN("[kinoPlan]: search t: %lf, opt t: %lf", t_search, t_opt);
...
void FastPlannerManager::waypointsReplanAlongTour(const vector<Eigen::Vector3d>& tour,
...
#ifdef DEBUG
  std::cout << "[trajPlan]:duration: " << duration <<
         ", seg_num: " << seg_num << ", dt: " << dt << std::endl;
#endif
...
#ifdef DEBUG
  std::cout<<"optimization finished, ctrl pts: "<< ctrl_pts.rows() <<std::endl;
#endif
  local_data_.position_traj_.setUniformBspline(ctrl_pts, pp_.bspline_degree_, dt);
#ifdef DEBUG
  ...
  updateTrajInfo();
#ifdef DEBUG
  std::cout<<"get traj info"<<std::endl;
#endif
}
...
// SECTION yaw replanning
// plan a yaw trajectory for looking at waypoint time forward (forward_t=2) from (start_yaw)
void FastPlannerManager::planYawFrom(const Eigen::Vector3d& start_yaw) {
...
void FastPlannerManager::planYawToward(const Eigen::Vector3d& start_yaw, 
  ...
  std::cout << "[yawPlan]:yaw_dt: " << dt_yaw << ", start yaw: " << start_yaw3d.transpose() << std::endl;
  ...
  // Add waypoint constraints towards target
  vector<Eigen::Vector3d> waypts;
  vector<int> waypt_idx;
#ifdef DEBUG
  std::cout << "[DEBUG]:i: 0, wp: " << last_yaw/M_PI*180 << std::endl;
#endif


  // first step interpolation
  int first_step;
  Eigen::Vector3d first_end3d;
  while(true){
    static int i = 1;
    Eigen::Vector3d pd = target - local_data_.position_traj_.evaluateDeBoorT(i * dt_yaw);
    if (pd.norm() > 1e-6) {
  ...
  for (int i = 1; i < first_step; ++i)
  {
    Eigen::Vector3d waypt = (1 - double(i) / first_step) * start_yaw3d + double(i) / first_step *
    first_end3d;
    calcNextYaw(last_yaw, waypt(0));
    last_yaw = waypt(0);
#ifdef DEBUG
    std::cout << "[DEBUG]:i: " << i << ", wp: " << last_yaw/M_PI*180 << std::endl;
#endif
    waypts.push_back(waypt);
    waypt_idx.push_back(i);
  }

  for (int i = first_step; i < seg_num; ++i) {
    Eigen::Vector3d pd = target - local_data_.position_traj_.evaluateDeBoorT(i*dt_yaw);
    Eigen::Vector3d waypt;
    if (pd.norm() > 1e-6) {
      waypt(0) = atan2(pd(1), pd(0));
  ...
  // Final state
  Eigen::Vector3d dis = target - local_data_.position_traj_.evaluateDeBoorT(local_data_.duration_);
  double end_yaw = atan2(dis(1), dis(0));
  Eigen::Vector3d end_yaw3d(end_yaw, 0, 0);
  calcNextYaw(last_yaw, end_yaw3d(0));
  yaw.block<3, 1>(seg_num, 0) = states2pts * end_yaw3d;
#ifdef DEBUG
  std::cout << "[DEBUG]:i: 12, wp: " << end_yaw/M_PI*180 << ", but: " <<end_yaw3d(0)/M_PI*180<< std::endl;

  // Debug too high change of yaw
  if (fabs(wrapYaw(start_yaw3d[0] - end_yaw3d[0])) >= M_PI) {
    ROS_ERROR("Yaw translation too high!");
...
// plan a yaw trajectory for looking at waypoint time forward (forward_t=2) from (start_yaw) to (end_yaw)
void FastPlannerManager::planYawFromToWith(const Eigen::Vector3d& start_yaw, const double& end_yaw,
  ...
  if (lookfwd) {
    ...
    for (int i = first_step; i < seg_num; ++i) {
      ...
      if(tc + forward_t > local_data_.duration_){
        second_step = i+1;
      }
    }

    // end step interpolation
    Eigen::Vector3d s_yaw3d = Eigen::Vector3d(last_yaw,0.0,0.0);
    Eigen::Vector3d e_yaw3d = Eigen::Vector3d(end_yaw,0.0,0.0);
    calcNextYaw(last_yaw, e_yaw3d[0]);
    for (int i = second_step; i < seg_num; ++i)
    {
      Eigen::Vector3d waypt = (1 - double(i) / (seg_num-second_step+1)) * s_yaw3d + 
                                    double(i) / (seg_num-second_step+1) * e_yaw3d;
      calcNextYaw(last_yaw, waypt(0));
      last_yaw = waypt(0);
#ifdef DEBUG
      std::cout << "[DEBUG]:i: " << i << ", wp: " << last_yaw/M_PI*180 << std::endl;
...
#ifdef DEBUG
    std::cout << "[DEBUG]:i: 12, wp: " << end_yaw/M_PI*180 << ", but: " <<e_yaw3d[0]/M_PI*180<< std::endl;
#endif
  }
...
#ifdef DEBUG
    std::cout << "[DEBUG]:i: n, wp: " << end_yaw/M_PI*180 << ", but: " <<end_yaw3d(0)/M_PI*180<< std::endl;
#endif
  }
  ...
  // Debug too high change of yaw
  if (fabs(wrapYaw(start_yaw3d[0] - end_yaw3d[0])) >= M_PI) {
    ROS_ERROR("[DEBUG]:Yaw translation too high!");
    std::cout << "[DEBUG]:start yaw: " << start_yaw3d[0] << ", end yaw" << end_yaw3d[0] << std::endl;
  }
#endif
