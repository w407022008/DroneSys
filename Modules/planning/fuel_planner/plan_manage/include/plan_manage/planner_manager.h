...
public:
  ...
  bool kinodynamicReplanFromTo(const Eigen::Vector3d& start_pt, const Eigen::Vector3d& start_vel,
                         const Eigen::Vector3d& start_acc, const Eigen::Vector3d& end_pt,
                         const Eigen::Vector3d& end_vel, const double& time_lb = -1);
  void waypointsReplanAlongTour(const vector<Eigen::Vector3d>& tour, const Eigen::Vector3d& cur_vel,
  ...
  void planYawFrom(const Eigen::Vector3d& start_yaw);
  void planYawToward(const Eigen::Vector3d& start_yaw, const Eigen::Vector3d& target, const double& relax_time=0.0);
  void planYawFromToWith(const Eigen::Vector3d& start_yaw, const double& end_yaw, bool lookfwd=false,
                      const double& relax_time=0.0);
