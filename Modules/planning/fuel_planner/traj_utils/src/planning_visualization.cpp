...
PlanningVisualization::PlanningVisualization(ros::NodeHandle& nh) {
  ...
  next_frontier_pub_ = node.advertise<visualization_msgs::Marker>("/planning_vis/next_frontier", 10000);
  pubs_.push_back(next_frontier_pub_);
...
void PlanningVisualization::drawArrows(const vector<Eigen::Vector4d>& list, const double& scale, const Eigen::Vector4d& color, const string& ns,
  ...
  for (int i = 0; i < int(list.size()); i++) { 
    ...
    double roll = 0, pitch = 0, yaw = list[i](3);    
    Eigen::Quaternionf q;
...
void PlanningVisualization::drawArrow(const Eigen::Vector4d& list, const double& scale, const Eigen::Vector4d& color, const string& ns,
                                       const int& id, const int& pub_id) {
  // Clean old marker
  visualization_msgs::Marker mk;
  ...
  // pub new marker
  //Roll pitch and yaw in Radians
  double roll = 0, pitch = 0, yaw = list(3);    
  Eigen::Quaternionf q;
  q = Eigen::AngleAxisf(roll, Eigen::Vector3f::UnitX())
      * Eigen::AngleAxisf(pitch, Eigen::Vector3f::UnitY())
      * Eigen::AngleAxisf(yaw, Eigen::Vector3f::UnitZ());
  mk.pose.position.x = list(0);
  mk.pose.position.y = list(1);
  mk.pose.position.z = list(2);
  mk.pose.orientation.w = q.w();
  mk.pose.orientation.x = q.x();
  mk.pose.orientation.y = q.y();
  mk.pose.orientation.z = q.z();
