...
private:
  ...
  ros::Publisher next_frontier_pub_; // 7, next fronter to visit
...
public:
  ...
  void drawArrows(const vector<Eigen::Vector4d>& list, const double& scale, const Eigen::Vector4d& color, const string& ns, 
                  const int& id, const int& pub_id);
  void drawArrow(const Eigen::Vector4d& list, const double& scale, const Eigen::Vector4d& color, const string& ns, 
                  const int& id, const int& pub_id);
  // Deprecated
  // draw basic shapes
