...
ViewNode::ViewNode(const Vector3d& p, const double& y, const Vector3d& a) {
  pos_ = p;
  yaw_ = y;
  average_ = a;
  parent_ = nullptr;
...
double ViewNode::computeCost(const Vector3d& p1, const Vector3d& p2, const double& y1, const double& y2,
  ...
  double pos_cost = ViewNode::searchPath(p1, p2, path) / vm_;// vm_   3m/s  i.e. time cost. path is obtained by A*
  ...
  Vector3d dir = (p2 - p1).normalized();
  Vector3d vdir = Vector3d(cos(y1),sin(y1),0.0);
  diff = acos(vdir.dot(dir));
  yaw_cost += w_dir_ * diff;
