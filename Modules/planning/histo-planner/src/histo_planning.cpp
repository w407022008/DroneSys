...
void Histo_Planner::init(ros::NodeHandle& nh)
{
  ...
  /* -------------- B-Spline -------------*/
  nh.param("bspline/limit_vel", UniformBspline::limit_vel_, -1.0);                         // velocity limit
  nh.param("bspline/limit_acc", UniformBspline::limit_acc_, -1.0);                         // accel limit
  /* -------------- Planner -------------*/
  ...
  */
  ...
  UniformBspline::limit_omega_ = yaw_rate;
  UniformBspline::yaw_track = yaw_tracking_mode>0;
  ...
  */
  ...
  // a_pub = nh.advertise<std_msgs::Float32>("/aa", 10);
  // b_pub = nh.advertise<std_msgs::Float32>("/bb", 10);
  // c_pub = nh.advertise<std_msgs::Float32>("/cc", 10);
  // d_pub = nh.advertise<std_msgs::Float32>("/dd", 10);
  ...
  // Initialising the environment, if use ESDF map
  ...
  // bspline_optimizer_->setEnvironment(sdf_map_);
...
void Histo_Planner::drone_state_cb(const drone_msgs::DroneStateConstPtr& msg)
{
    ...
    // if (sdf_map_->sdfState()) sdf_map_->set_odom(Drone_odom);
}
...
void Histo_Planner::localcloudCallback(const sensor_msgs::PointCloud2ConstPtr &msg)
{
  ...
  // if (sdf_map_->sdfState()) sdf_map_->set_local_map_pcl(pcl_ptr);
...
bool Histo_Planner::generateTrajectory(Eigen::Vector3d start_pos_, Eigen::Vector3d start_vel_, Eigen::Vector3d start_acc_, Eigen::Vector3d end_pos_, Eigen::Vector3d end_vel_, Eigen::Vector3d end_acc_)
{
  ...
  */
  ...
  UniformBspline::cubicSamplePts_to_BsplineCtlPts(samples, time_interval, control_pts);
  traj_init_ = UniformBspline(control_pts, 3, time_interval);// B-Spline Unoptimised
  ...
  UniformBspline pos = UniformBspline(control_pts, 3, time_interval);// get B-Spline 
  double tm, tmp, to, tn;
  ...
  // if (sdf_map_->sdfState()) cout << "[SDFMap]: ESDF processing takes " << sdf_map_->getUpdateDelay() << " [ms].\n" << endl;
...
int Histo_Planner::safetyCheck()
{
  ...
  // if (sdf_map_->sdfState()){
  //   if ((sdf_map_->getDistTrilinear(pos) < forbidden_range+0.05))
  //   {
  //     goal_ready = false;
  //     cout << "[safetyCheck]: current position in collision. " << endl;
  ...
  if(check_goal_safty && goal_regenerate_mode==1){
    ...
    // if (sdf_map_->sdfState())
    //   dist = sdf_map_->getDistWithGradTrilinear(goal_pos,grad);
    // else
    ...
    if (dist < forbidden_plus_safe_distance)
    {
      ...
      for (int r = 1; r <= 6; r++)
      {
        ...
        // if (sdf_map_->sdfState())
        //   dist = sdf_map_->getDistTrilinear(new_pos);
        // else
  ...
  // [Not recommended]: For voluminous obstacles, it is possible that the point found falls inside obs's body
    ...
    double dist;
    // if (sdf_map_->sdfState())
    //   dist = sdf_map_->getDistTrilinear(goal_pos);
    // else
      ...
      for (int theta = 0; theta < 13; theta++)
      {
        for (int nz = 0; nz <= 4; nz++)
        {
          for (int r = 1; r <= 4; r++)
          {
            ...
            // if (sdf_map_->sdfState())
            //   dist = sdf_map_->getDistTrilinear(Eigen::Vector3d(new_x, new_y, new_z));
            // else
...
bool Histo_Planner::checkTrajCollision(double t_start, double t_end, double safe_range, double dt_min, double dt_max, bool force)
{
  ...
  for (double t = start_check_time; t <= end_check_time; t += dt)
  {
    ...
    double dist;
    // if (sdf_map_->sdfState())
    //   dist = sdf_map_->getDistTrilinear(pos);
    // else
...
// std_msgs::Float32 ddd;
// ddd.data=(desired_yaw-cur_yaw)*57.29;
// d_pub.publish(ddd);
        static int count=0;
...
// std_msgs::Float32 aaa,bbb,ccc;
// aaa.data=desired_yaw*57.29;
// bbb.data=yaw_ref_comd*57.29;
// ccc.data=cur_yaw*57.29;
// a_pub.publish(aaa);
// b_pub.publish(bbb);
// c_pub.publish(ccc);
