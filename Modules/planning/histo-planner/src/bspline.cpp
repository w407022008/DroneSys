...
{
double Bspline::limit_vel_;
double Bspline::limit_acc_;
double Bspline::limit_omega_;
bool Bspline::yaw_track;

// control points is a (n+1)x3 matrix
Bspline::Bspline(Eigen::MatrixXd points, int order, double interval_, bool zero)
{
  this->p_ = order;
...
void Bspline::setKnot(Eigen::VectorXd knot)
{
  this->u_ = knot;
}

Eigen::VectorXd Bspline::getKnot()
{
  return this->u_;
}

void Bspline::getTimeSpan(double& um, double& um_p)
{
  um = this->u_(this->p_);
  um_p = this->u_(this->m_ - this->p_);
}

Eigen::Vector3d Bspline::evaluateDeBoor(double t)
{
...
Eigen::Vector3d Bspline::getLocation(double t_cur, double dist_forward)
{
  double length = 0.0;
...
Eigen::MatrixXd Bspline::getDerivativeControlPoints()
{
...
Bspline Bspline::getDerivative()
{
  ...
  Bspline derivative = Bspline(ctp, p_ - 1, this->interval_, false);
...
bool Bspline::checkFeasibility(bool show, double ts)
{
  // SETY << "[Bspline]: total points size: " << control_points_.rows() << endl;

// Option 1: Continuous sampling
  bool fea = true;
  Bspline vel = getDerivative();
  Bspline acc = getDerivative().getDerivative();
...
bool Bspline::reallocateTime(bool show, double ts)
{
  // SETY << "[Bspline]: total points size: " << control_points_.rows() << endl;
  // cout << "origin knots:\n" << u_.transpose() << endl;
  bool fea = true;
...
void Bspline::cubicSamplePts_to_BsplineCtlPts(vector<Eigen::Vector3d> samples, double ts, Eigen::MatrixXd& control_pts)
{
...
void Bspline::BsplineParameterize(const double& ts, const vector<Eigen::Vector3d>& point_set,
...
void Bspline::getMeanMinMax(double& mean_, double& min_, double& max_, double t_start_, double t_end_, int sample_num)
{
...
void Bspline::getVelMeanMinMax(double& mean_v, double& min_v, double& max_v, double t_start_, double t_end_, int sample_num)
{
  Bspline vel = getDerivative();
  double tm, tmp, t_interval;
  if(t_start_ == -1.0 && t_end_ == -1.0){
...
void Bspline::getAccMeanMinMax(double& mean_a, double& min_a, double& max_a, double t_start_, double t_end_, int sample_num)
{
  Bspline acc = getDerivative().getDerivative();
  double tm, tmp, t_interval;
  if(t_start_ == -1.0 && t_end_ == -1.0){
