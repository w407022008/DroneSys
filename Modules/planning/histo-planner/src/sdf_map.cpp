...
SDFMap::SDFMap(Eigen::Vector3d ori, double resolution, Eigen::Vector3d size) {
    ...
    fill(distance_buffer_.begin(), distance_buffer_.end(), MAX_DIS);
    fill(occupancy_buffer_.begin(), occupancy_buffer_.end(), 0.0);
}

void SDFMap::init(ros::NodeHandle &nh) {
    /* ---------- param in ENU_FRAME ---------- */
    nh.param("sdf_map/origin_x", origin_(0), -20.0);
    nh.param("sdf_map/origin_y", origin_(1), -20.0);
    nh.param("sdf_map/origin_z", origin_(2), 0.0);
    nh.param("sdf_map/max_ground_height", max_ground_height_, 0.1);
    nh.param("sdf_map/map_size_x", map_size_(0), 40.0);
    nh.param("sdf_map/map_size_y", map_size_(1), 40.0);
    nh.param("sdf_map/map_size_z", map_size_(2), 5.0);
    nh.param("sdf_map/ceil_height", ceil_height_, 2.0);

    nh.param("sdf_map/trigger", trigger, true);
    nh.param("sdf_map/update_rate_Hz", update_rate_, 10.0);
    nh.param("sdf_map/update_range", update_range_, 4.0);
    nh.param("sdf_map/resolution_sdf", resolution_sdf_, 0.2);
    nh.param("sdf_map/inflate_cloud_pub_", pub_inflate_cloud, false);
    nh.param("sdf_map/inflate", inflate_, 0.2);
    nh.param("sdf_map/radius_ignore", radius_ignore_, 0.2); // useless

    cout << "[SDFMap]: trigger (disable / enable): " << trigger << endl;
    if(!trigger) return;
    if(origin_(2) < max_ground_height_) origin_(2) = max_ground_height_; 
    if(ceil_height_ > max_ground_height_) map_size_(2) = ceil_height_ - max_ground_height_;

    cout << "[SDFMap]: origin in ENU_FRAME: " << origin_.transpose() << endl;
    cout << "[SDFMap]: map size: " << map_size_.transpose() << endl;
    cout << "[SDFMap]: resolution: " << resolution_sdf_ << endl;

    /* ---------- sub / pub / timer ---------- */
    if(pub_inflate_cloud) inflate_cloud_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/histo_planner/sdf_map/inflate_cloud", 10);
    update_timer_ = nh.createTimer(ros::Duration(1.0/update_rate_), &SDFMap::updateCallback, this);
	...
	cout << "[SDFMap]: grid num: " << grid_size_.transpose() << endl;
    min_range_ = origin_;
    max_range_ = origin_ + map_size_;
    ...
    // initialize size of buffer
    int map_size = grid_size_(0) * grid_size_(1) * grid_size_(2);
    occupancy_buffer_.resize(map_size);
    distance_buffer_.resize(map_size);
    distance_buffer_neg_.resize(map_size);
    tmp_buffer1_.resize(map_size);
    tmp_buffer2_.resize(map_size);

    fill(distance_buffer_.begin(), distance_buffer_.end(), MAX_DIS);
    fill(distance_buffer_neg_.begin(), distance_buffer_neg_.end(), MAX_DIS);
    fill(occupancy_buffer_.begin(), occupancy_buffer_.end(), 0);
}

void SDFMap::set_local_map_pcl(pcl::PointCloud<pcl::PointXYZ>::Ptr &pcl_ptr)
{
	latest_cloud_ = *pcl_ptr;
	new_map_ = true;
}

void SDFMap::set_odom(nav_msgs::Odometry cur_odom)
{
	odom_ = cur_odom;
...
void SDFMap::updateCallback(const ros::TimerEvent &e) {
    if (!trigger || !new_map_ || !have_odom_)
        return;
    ...
    exec_num++;
    new_map_ = false;have_odom_ = false;
	if(exec_num % 100 == 0) cout << "[SDFMap]: get " << latest_cloud_.points.size() << " points cloud." << endl;

    /* ---------- inflate cloud and insert to SDFMap ---------- */
    Eigen::Vector3d disp(update_range_, update_range_, update_range_ / 2.0);
    this->resetUpdatedRange(center - disp, center + disp, true); // reset the whole space

    if(pub_inflate_cloud) {
...
//                    	if(fabs(x)!=ifn && fabs(y)!=ifn && fabs(z)!=ifn) continue;
                        ...
                        p3d_inf(2) = pt_inf.z = pt.z + z * resolution_sdf_;
                        this->setOccupancy(p3d_inf);
                        if (pub_inflate_cloud && isInMap(p3d_inf))
    ...
    for (double cx = center(0) - update_range_; cx <= center(0) + update_range_; cx += resolution_sdf_)
        for (double cy = center(1) - update_range_; cy <= center(1) + update_range_; cy += resolution_sdf_){
            this->setOccupancy(Eigen::Vector3d(cx, cy, origin_(2)+resolution_sdf_/2));
            if (ceil_height_ > max_ground_height_)
            	this->setOccupancy(Eigen::Vector3d(cx, cy, map_size_(2)-resolution_sdf_/2));
        }
    ...
    update_delay = elapsed_seconds.count();
//    cout << "ESDF processing takes " << elapsed_seconds.count() << " [ms]." << endl;
    
    map_valid_ = true;
}

void SDFMap::resetUpdatedRange(Eigen::Vector3d min_pos, Eigen::Vector3d max_pos, bool reset_all_) {
	/* Reset the whole space */
	if(reset_all_){
//		static int index = 0;
//		if (index==0)
//			index++;
			...
			for (int x = min_vec_(0); x <= max_vec_(0); ++x)
				for (int y = min_vec_(1); y <= max_vec_(1); ++y)
					for (int z = min_vec_(2); z <= max_vec_(2); ++z) {
					    ...
					    distance_buffer_[x * grid_size_(1) * grid_size_(2) + y * grid_size_(2) + z] = MAX_DIS;
					}
	}
...
bool SDFMap::isInMap(Eigen::Vector3d pos) {
    if (pos(0) < min_range_(0) + 1e-4 || pos(1) < min_range_(1) + 1e-4 || pos(2) < min_range_(2) + 1e-4) {
        // cout << "smaller than min range!" << pos.transpose() << endl;
        return false;
    }
