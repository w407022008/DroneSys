...
{
  node = nh;
  init_traj_pub = node.advertise<visualization_msgs::Marker>("/fast_planner/initial_trajectory", 10);
  opt_traj_pub = node.advertise<visualization_msgs::Marker>("/fast_planner/optimized_trajectory", 10);
}
...
{
  displaySphereList(init_traj_pub, path, resolution, color, PATH + id % 100);
}
