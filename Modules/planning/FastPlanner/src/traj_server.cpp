...
void bsplineCallback(drone_msgs::BsplineConstPtr msg) {
...
void cmdCallback(const ros::TimerEvent& e) 
{
  ...
  drawState(pos, vel, 0, Eigen::Vector4d(0, 1, 0, 1));// vel --> green
  drawState(pos, acc, 1, Eigen::Vector4d(0, 0, 1, 1));// acc --> blue
...
int main(int argc, char** argv) 
{
  ...
  ros::Subscriber bspline_sub = node.subscribe("/fast_planner/bspline", 10, bsplineCallback);
  ...
  pos_cmd_pub = node.advertise<drone_msgs::PositionReference>("/fast_planner/position_cmd", 50);
  
  state_pub = node.advertise<visualization_msgs::Marker>("/fast_planner/state", 10);
  
  // 发布轨迹控制指令，无人机实际轨迹
  ros::Timer vis_timer = node.createTimer(ros::Duration(0.2), visCallback);
  traj_pub = node.advertise<visualization_msgs::Marker>("/fast_planner/traj", 10);
