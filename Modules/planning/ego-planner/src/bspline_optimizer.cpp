...
namespace ego_planner
{
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    std::chrono::time_point<std::chrono::system_clock> t0 = std::chrono::system_clock::now(), t1, t2;
    ...
    do
    {
      ...
      std::chrono::duration<double, std::milli> elapsed_seconds = t2 - t1; 
      double time_ms = elapsed_seconds.count();
      elapsed_seconds = t2 - t0; 
      double total_time_ms = elapsed_seconds.count();
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        if (!flag_occ)
        {
          printf("\033[33miter(+1)=%d,time(ms)=%5.3f,total_t(ms)=%5.3f,cost=%5.3f\n\033[0m", iter_num_, time_ms, total_time_ms, final_cost);
          success = true;
        }
        else // restart
        {
          ...
          // printf("\033[33miter(+1)=%d,time(ms)=%5.3f,keep optimizing\n\033[0m", iter_num_, time_ms);
        }
      }
