...
namespace ego_planner
{
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    std::chrono::duration<double, std::milli> elapsed_seconds = std::chrono::system_clock::now() - t_start;
    t_init = elapsed_seconds.count();
    ...
    elapsed_seconds = std::chrono::system_clock::now() - t_start;
    t_refine = elapsed_seconds.count();

    // save planned results
    updateTrajInfo(pos, ros::Time::now());

    printf("\033[32mtotal_time(ms)=%5.3f,init(ms)=%5.3f,optimize(ms)=%5.3f,refine=%5.3f\n\033[0m", (t_init+t_opt+t_refine), t_init, t_opt, t_refine);
